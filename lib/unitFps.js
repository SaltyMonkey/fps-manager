	...
	constructor(deps) {
		...
		const modifyPlayerEvent = (event) => {
			...
			if (data.activeMode.blockPlayersAppearance) {
				...
				if (data.activeMode.blockPlayersAppearance.allLook) {
					res.event.weapon = [10001, 10002, 10003, 10004, 10005, 10006, 10007, 10008, 80396, 55005, 82005, 58171, 59053][res.event.templateId % 100 - 1]; //temp fix from Risenio fps utils TODO: remove hardcoded ids
		...
		const isProjectileBlockable = (event) => {
			if (data.activeMode.blockProjectiles && data.activeMode.blockProjectiles.players) {
				if (event.skill.npc || data.whitelistProjectiles.has(event.skill.id)) return false;
