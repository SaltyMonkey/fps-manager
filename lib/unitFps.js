	...
	constructor(deps) {
		...
		const isAbnormalBlockable = (event) => {
			if (data.activeMode.blockBuffs) {
				if (isEntityBlocked(event.target) || (data.activeMode.blockBuffs.area && !isMe(event.target) && tracker.spawnedPlayers.has(event.target))) return true;
		...
		//mod.hook(...proto.getData("S_ITEM_EXPLOSION_RESULT"), HOOK_SETTINGS.LAST, itemsExplosionHandler); //TODO: NEED MAP
