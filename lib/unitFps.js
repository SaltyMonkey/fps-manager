	...
	*/
	constructor(deps) {
		...
		const isDropBlockable = (event) => {
			if (data.activeMode.blockDropItems && data.activeMode.blockDropItems) {
		...
		const isServantBlockable = (event) => {
			if (data.activeMode.blockServants) {
				if (data.activeMode.blockServants.me && isMe(event.ownerId))
					return true;
				else if (data.activeMode.blockServants.area)
					return true;
			}
			return false;
		};
		...
		const isSummonsBlockable = (event) => {
			if (data.activeMode.blockSummons) {
				if (data.activeMode.blockSummons.my && isMe(event.owner)) {
					return true;
				}
		...
		const projectileHandler = (event) => {
			...
			if (data.activeMode.blockProjectiles && data.activeMode.blockProjectiles) {
				if (data.activeMode.blockProjectiles.players && tracker.spawnedPlayers.has(event.gameId)) return false;
			}
		};

		const blockAdditionalSkillEffectsHandler = (event) => {
			if (isEntityBlocked(event.gameId)) return false;
			if (data.activeMode.blockAdditionalSkillEffects && data.activeMode.blockAdditionalSkillEffects) {
				if (data.activeMode.blockAdditionalSkillEffects.players && tracker.spawnedPlayers.has(event.gameId)) return false;
				if (data.activeMode.blockAdditionalSkillEffects.own && isMe(event.gameId)) return false;
			}
		};
		...
		const throttlePacketsHandler = (name, code, payload) => {
			...
			let needBlock = (data.activeMode.throttleSomePackets && throttlePacketsData[name] && (Buffer.compare(throttlePacketsData[name], payload) === 0));
		...
		const abnormalsHandler = (event) => {
			if (data.activeMode.blockBuffs) {
				if ((data.activeMode.blockBuffs.area && !isMe(event.target)) || isEntityBlocked(event.target)) return false;

				if (data.activeMode.blockBuffs.huntRewards && data.huntingRewardsBuffs.has(event.id)) return false;
				if (data.activeMode.blockBuffs.shapeChange && data.shapeChangersBuffs.has(event.id)) return false;
				if (data.activeMode.blockBuffs.effects && data.onScreenEffectsBuffs.has(event.id)) return false;
			}
		};
		...
		const fontSwapHandler = () => (data.activeMode.blockNumbersPopups && data.activeMode.blockNumbersPopups.damage ? false : undefined);
		...
		//mod.hook(...proto.getData("S_SOCIAL_CANCEL"), HOOK_SETTINGS.LAST, dropIfBlockedHandler);
		...
		deps.data.on(EVENTS.RECONFIGURE, () => {
			...
			for (const [key, value] of tracker.spawnedDrop.entries()) {
				assert.ok(Object.keys(value).length > 3, "reconfigure event: wrong object drop");
			...
			for (const [key, value] of tracker.spawnedPlayers.entries()) {
				assert.ok(Object.keys(value).length > 3, "reconfigure event: wrong object players");
			...
			for (const [key, value] of tracker.spawnedServants.entries()) {
				assert.ok(Object.keys(value).length > 3, "reconfigure event: wrong object servants");
			...
			for (const [key, value] of tracker.spawnedSummons.entries()) {
				assert.ok(Object.keys(value).length > 3, "reconfigure event: wrong object summons");
			...
			for (const [key, value] of tracker.spawnedNPCS.entries()) {
				assert.ok(Object.keys(value).length > 3, "reconfigure event: wrong object NPCS");
				let needBlock = isSummonsBlockable(value);
