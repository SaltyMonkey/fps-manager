	...
	*/
	constructor(deps) {
		...
		deps.data.on(EVENTS.RECONFIGURE, (oldMode, newMode) => {
			...
			if (oldMode.blockCollections !== newMode.blockCollections && newMode.blockCollections === false) {
				for (const val of tracker.collections.values()) {
					mod.send(...proto.getData("S_SPAWN_COLLECTION"), val);
				}
			}
			//respawn servants back
			if (oldMode.blockOwnServants !== newMode.blockOwnServants && newMode.blockOwnServants === false) {
				for (const val of tracker.blockedServants.values()) {
					mod.send(...proto.getData("S_REQUEST_SPAWN_SERVANT"), val);
				}
			}
		});
	}
