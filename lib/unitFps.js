	...
	*/
	constructor(deps) {
		...
		const isUserBlockable = (event) => {
			if (data.activeMode.blockPlayers && data.activeMode.blockPlayersOptions) {
				if (data.activeMode.blockPlayersOptions.all) {
					...
					if (utils.arraysHasIntersect(event.name, data.activeMode.blockPlayersOptions.name)) {
						return true;
					}
				}
			}
		...
		const creatureChangeHpHandler = (event) => {
			if (data.activeMode.blockNumbersPopups && data.activeMode.blockNumbersPopupsOptions.hp && isMe(event.target)) {
		...
		const creatureChangeMpHandler = (event) => {
			if (data.activeMode.blockNumbersPopups && data.activeMode.blockNumbersPopupsOptions.mp && isMe(event.target)) {
		...
		const eachSkillResultHandler = (event) => {
			if (isMe(event.source) || isMe(event.owner)) {
				...
				if (data.activeMode.blockNumbersPopups && data.activeMode.blockNumbersPopupsOptions.damage) {
					// eslint-disable-next-line no-magic-numbers
		...
		const fontSwapHandler = () => (data.activeMode.blockNumbersPopups && data.activeMode.blockNumbersPopupsOptions.damage ? false : undefined);
