	...
	*/
	constructor(deps) {
		...
		const isSkillBlockable = (event) => {
			if(isMe(event.gameId)) return false;
		...
		const abnormalsHandler = (event) => {
			if (data.activeMode.blockBuffs) {
				if (isEntityBlocked(event.target) || (data.activeMode.blockBuffs.area && !isMe(event.target) && !tracker.spawnedNPCS.has(event.target))) return false;
