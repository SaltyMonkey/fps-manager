	...
	constructor(deps) {
		...
		const npcHandler = (type, event) => {
			switch (type) {
				case (ENTITY_HOOK_TYPE.DESPAWN):
					...
					break;
				case (ENTITY_HOOK_TYPE.SPAWN):
					if(tracker.summons.has(event.gameId) && isSummonsBlockable(event)) {
		...
		const blockAdditionalSkillEffectsHandler = (event) => {
			...
			if (data.activeMode.blockAdditionalSkillEffects && data.activeMode.blockAdditionalSkillEffectsOptions) {
				if (data.activeMode.blockAdditionalSkillEffectsOptions.players && tracker.users.has(event.gameId)) return false;
		...
		const achievementHandler = () => ((data.activeMode.blockAchievementSpamInCombat && meInCombat()) ? false : undefined);
		...
		deps.data.on(EVENTS.RECONFIGURE, () => {
			...
			if (data.prevMode.blockCollections !== data.activeMode.blockCollections) {
				switch (data.activeMode.blockCollections) {
					case (true):
						for (const value of tracker.collections.values()) {
			...
			for (const [key, value] of tracker.drop.entries()) {
			...
			for (const [key, value] of tracker.users.entries()) {
			...
			for (const [key, value] of tracker.servants.entries()) {
			...
			for (const [key, value] of tracker.summons.entries()) {
			...
			for (const [key, value] of tracker.npcs.entries()) {
				let needBlock = isSummonsBlockable(value);
