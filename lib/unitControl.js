...
class unitControl {
	...
	*/
	constructor(deps) {
		...
		if(deps.mod.settings.customCommandTag && deps.mod.settings.customCommandTag.length)
			prefixes.push(deps.mod.settings.customCommandTag);
		...
		const autoSettingHandler = () => {
			deps.mod.settings.applyTriggers = !deps.mod.settings.applyTriggers;
			deps.mod.command.message(`Triggers ${deps.mod.settings.applyTriggers ? "a" : "dea"}ctivated.`);
		};
		...
		const debugSettingsHandler = () => {
			...
			deps.mod.command.message(`Debug ${deps.mod.settings.debugMode ? "a" : "dea"}ctivated.`);
		};
		...
		if(deps.mod.settings.enableCommands)
