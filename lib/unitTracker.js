	...
	constructor(deps) {
		...
		const userMounted = (type, event) => {
			if (this.users.has(event.gameId)) {
				...
				switch (type) {
					case (ENTITY_HOOK_TYPE.DESPAWN):
						data = this.users.get(event.gameId);
						data.mount = 0;
		...
		const updateLoc = (data, loc) => data.loc = loc;
		...
		const userLocationHandler = (event) => {
			if (this.users.has(event.gameId)) {
				this.users.set(event.gameId, updateLoc(this.users.get(event.gameId), event.loc));
			}
		};

		const npcLocationHandler = (event) => {
			if (this.npcs.has(event.gameId)) {
				this.npcs.set(event.gameId, updateLoc(this.npcs.get(event.gameId), event.loc));
			}
			else if (this.summons.has(event.gameId)) {
				this.summons.set(event.gameId, updateLoc(this.summons.get(event.gameId), event.loc));
			}
			else if (this.servants.has(event.gameId)) {
				this.servants.set(event.gameId, updateLoc(this.servants.get(event.gameId), event.loc));
			}
		};
		...
		mod.hook(...proto.getData("S_USER_LOCATION_IN_ACTION"), HOOK_SETTINGS.FIRST, userLocationHandler);
