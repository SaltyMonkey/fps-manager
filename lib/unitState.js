...
class unitState extends EventEmitter {
	...
	constructor(deps) {
		...
		this.__mod.mod.queryData("/S1ActionScripts@type=?/Script/", ["Teleport"], true, false, ["id"]).then(result => {
	...
	Init() {
		...
		this.__mod.hook(...this.__proto.getData("S_LOAD_TOPO"), (event) => {
			...
			if (this.__mod.game.me.inBattleground && this.preset.directives.triggerBG) {
				...
				this.__mod.command.message(`Mode was changed to "${this.preset.directives.triggerBG}".`);
			}
				...
				this.__mod.command.message(`Mode was changed to "${this.preset.directives.triggerCU}".`);
			}
				...
				if(mode) {
					...
					this.__mod.command.message(`Mode was changed to ${name}`);
				}
			}
				...
				this.__mod.command.message(`Mode was changed to "${this.preset.directives.triggerOW}".`);
			}
		...
		this.__mod.hook("S_FIELD_EVENT_ON_ENTER", RAW_HOOK_MODE.EVENT, () => {
			...
			this.__mod.command.message(`Mode was changed to "${this.preset.directives.triggerGuardians}".`);
		...
		this.__mod.hook("S_FIELD_EVENT_ON_LEAVE", RAW_HOOK_MODE.EVENT, () => {
			...
			this.__mod.command.message("Mode was changed back");
	...
	_eventTriggerPartial() {
		console.log("DEBUG: unitState -> _eventTriggerPartial");
