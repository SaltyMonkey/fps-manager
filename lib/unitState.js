...
class unitState extends EventEmitter {
	...
	Init() {
		...
		this.__mod.hook("S_LOGIN", RAW_HOOK_MODE.EVENT, HOOK_SETTINGS.FIRST, () => {
			...
			if(this.__mod.manager.isLoaded("shaker")) {
				this.__mod.warning("Camera shake effects will not be applied as the Shaker mod is installed. Please use \"/toolbox shaker\" to configure camera shaking behavior");
